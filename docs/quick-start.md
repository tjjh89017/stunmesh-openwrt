...
### Configure `/etc/config/stunmesh-agent` on the router

Paste the four printed constants (plus the identity key file's path)
into `/etc/config/stunmesh-agent` on the router:
...
never goes in this file. `/etc/config/stunmesh-agent` is mode 0644 (world
